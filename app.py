...
# --- 网页上传组件 ---
uploaded_file = st.file_uploader("📂 请上传 VC ASIN 数据表 (Excel 格式)", type=['xlsx'])

if uploaded_file is not None:
    with st.spinner('正在执行数据清洗与预警分析...'):
        # 1. 读取数据 (跳过第0行合并的日期，读取第1行作为列名)
        ...
        if len(date_columns_sorted) < 2:
            ...
            for index, row in df.iterrows():
                ...
                # 处理空值
                sales_latest = 0 if pd.isna(sales_latest) else sales_latest
                sales_prev = 0 if pd.isna(sales_prev) else sales_prev
                
                avg_sales = (sales_latest + sales_prev) / 2
                sales_change = sales_latest - sales_prev
                
                # 模拟 L30D 均销 (此处应有实际 L30D 计算，为演示简化取当前均值)
                l30d_avg = avg_sales 
